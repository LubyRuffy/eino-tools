...
func TestTool_InvokableRun_TimesOut(t *testing.T) {
	tl, err := New(Config{})
	require.NoError(t, err)

	result, runErr := tl.InvokableRun(context.Background(), `{"command":"sleep 1","timeout_ms":10}`)
	require.NoError(t, runErr)
	assert.Contains(t, result, `"failed":true`)
	assert.Contains(t, result, "command timeout or canceled")
}
...
func TestTool_InvokableRun_ImplementsBaseTool(t *testing.T) {
	tl, err := New(Config{})
	require.NoError(t, err)
