...
const ToolName = "run_bash_command"
...
func (t *Tool) Execute(ctx context.Context, params Params) (map[string]interface{}, error) {
	...
	if blockedDomain := t.BlockedProtectedDomainForCommand(params.Command); blockedDomain != "" {
		return nil, fmt.Errorf(
			"目标域名 %s 已命中过 Cloudflare 验证，禁止继续使用直连 HTTP 脚本（curl/wget/requests/httpx）以避免验证失效；请改用 browser_automation 工具继续抓取",
			blockedDomain,
		)
	}
	...
	baseDir, err := resolveBaseDir(t.defaultBaseDir, params.BaseDir)
	if err != nil {
		return nil, err
	}
	workDir := baseDir
	if strings.TrimSpace(params.CWD) != "" {
		resolved, err := resolvePathWithin(baseDir, params.CWD, t.allowedPaths)
		if err != nil {
	...
	if detected && t.challengeHandler != nil {
		...
		if _, stillBlocked := cloudflare.DetectFromCommandOutput(params.Command, retryStdout, retryStderr); stillBlocked {
			return nil, fmt.Errorf("Cloudflare challenge still exists after manual verification, please retry with browser_automation/fetch_url: %s", strings.TrimSpace(challengeURL))
		}
